...
f = open('/Users/tal/Code/tweets/data/js/tweets/2013_06.js', 'r')
...
for tweet in data:
    ...
    time = time.split()
    time = [time[1],int(time[2]),int(time[3][:2]+time[3][3:5]),int(time[5])] #Month,Day,HourMinute,Year
    ...
    if time[2] < 0: # Back to yesterday
        ...
        if time[1] < 1: # Back to last month
            time[0] = months[month_info[time[0]][0]-1 % 12]  ######################### Test this
    ...
    print(sst)
    tweets.append(sst)
    # sort first on day, then on time
